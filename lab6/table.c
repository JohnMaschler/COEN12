...
void quicksort(int, int, int (*compare)(), void *array[]);
int partition(int, int, int (*compare)(), void *array[]);
...
void *getElements(SET *sp)
{
    ...
    quicksort(0, j-1, sp->compare, elts);//call quicksort

    return elts;
}

/*
O(n logn) on average, worst case O(n^2)
quicksort function that is a recursive sorting function that picks a pivot point and recursively sorts the left and right subarrays
*/
void quicksort(int low, int high, int (*compare)(), void *array[])
{
    int location;
    if (low<high)
    {
        location = partition(low, high, compare, array);
        quicksort(low, location-1, compare, array);
        quicksort(location+1, high, compare, array);
    }
}

/*
O(log n)
function that partitions the array for the quicksort function to use
*/
int partition(int low, int high, int (*compare)(), void *array[])
{
    void *temp;
    int i;
    int x = low;
    for (i=low; i<high; i++)
    {
        if ((compare)(array[i], array[high])<0)
        {
            temp = array[x];
            array[x]=array[i];
            array[i]=temp;
